...
A2A 客户端：代表用户或其他系统向 A2A 服务器发起请求的应用程序或代理。

A2A 服务器（远程代理）：公开符合 A2A 的 HTTP 端点、处理任务并提供响应的代理或代理系统。

代理卡： A2A 服务器发布的 JSON 元数据文档，描述其身份、功能、技能、服务端点和身份验证要求。

消息：客户端和远程代理之间的通信回合，具有role（“用户”或“代理”）并包含一个或多个Parts。

任务： A2A 管理的基本工作单元，由唯一 ID 标识。任务是有状态的，并按照定义的生命周期进行。

部分：消息或工件中内容的最小单位（例如，，，TextPart）。FilePartDataPart

工件：代理执行任务后生成的输出（例如，文档、图像、结构化数据），由组成Parts。

流式传输（SSE）：通过服务器发送事件传递的任务（状态变化、工件块）的实时增量更新。

推送通知：通过服务器发起的 HTTP POST 请求向客户端提供的 webhook URL 传递异步任务更新，适用于长时间运行或断开连接的情况。
...
今天，听了公开课，但英语水平还是差太多了，听的懵懵懂懂的，打算有了回放以后再好好研究。
