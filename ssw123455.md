...
\### Building a Trustless Agent Network: A Journey of Learning by Doing and Reflecting

This note chronicles my recent progress in understanding and building a decentralized agent system based on the ERC-8004 standard. It highlights key learnings from hands-on implementation and subsequent reflection on the broader system architecture and future roadmap.

\---

\### October 22: Learning by Doing – On-chain Agent Registration

My journey began with the practical steps of bringing an agent onto the blockchain. The core idea is to enable agents to have an on-chain identity, allowing for trustless interaction.

\#### 1. Understanding the Ecosystem: \[\[ISEK\]\] and \[\[chaoschain\]\]

\[\[ISEK\]\] serves as an early platform for implementing the ERC-8004 standard, focusing on building a decentralized Agent-to-Agent (A2A) network. It provides a framework for various AI agents to connect, communicate (via gRPC/HTTP2), share context, and collaborate on complex tasks without central control. While ERC-8004 defines the rules and infrastructure (identity, reputation, validation), ISEK is the application layer that brings these agents to life, moving them from isolated tools to collaborative ecosystem participants. The key takeaway here is that agents, once registered, communicate directly A2A, with external LLM services providing inference but not routing.

\#### 2. The Registration Process: Getting an Agent On-Chain

The process of registering an agent involves several practical steps, culminating in an agent's identity being minted as an ERC-721 NFT on the blockchain.

\* \*\*\[\[注册到registor中\]\]\*\*: The fundamental step is to prepare a "registration file" in JSON format, which describes the agent's capabilities and endpoints. This file needs to be accessible via IPFS or HTTPS.

\* \*\*\[\[选网与拿到合约地址\]\]\*\*: First, I selected a network (e.g., local Anvil or a testnet like Ethereum Sepolia). ERC-8004 reference implementations provide consistent contract addresses across various testnets for IdentityRegistry, ReputationRegistry, and ValidationRegistry. Setting up the environment involved configuring RPC URLs and private keys securely.

\* \*\*\[\[托管注册文件并拿到tokenURI\]\]\*\*: The prepared JSON registration file was then uploaded to IPFS (recommended for content addressing) or an HTTPS server, yielding a `tokenURI` (e.g., `ipfs://Qm.../registration.json`). This URI points to the agent's metadata.

\* \*\*Calling the `IdentityRegistry`\*\*: Using `ethers.js`, I connected to the chosen network and interacted with the deployed `IdentityRegistry` contract. The `register(string tokenURI)` function was called, passing the `tokenURI` to mint the agent's NFT. A crucial learning point was ensuring the wallet had sufficient funds for gas and that the ABI correctly exposed all necessary contract methods.

\*\*Key Insight:\*\* The process revealed that a simple Agent Card JSON containing an interactive endpoint URL is essentially all that's needed to get an agent registered on-chain, establishing its foundational identity within the ERC-8004 ecosystem.

\---

\### October 23: Learning by Reflecting – System Architecture and Roadmap

After the hands-on experience, I stepped back to consolidate my understanding of the broader system and its future.

\#### 1. The Vision and Full Application Flow

My overarching vision is to assemble a decentralized agent system where services are secured on the blockchain, enabling trustless agent interaction through reputation and validation. This system aims to be a multi-purpose agent marketplace, allowing humans and agents to interact seamlessly.

The core interaction with an on-chain agent follows a closed loop: \*\*Discover Agent (via Identity Registry) → Execute Business (via A2A/MCP endpoints) → Record Feedback & Verification (on Reputation & Validation Registries) → Complete Payment (via wallet)\*\*.

\* \*\*Discovery & Resolution\*\*: Querying the Identity Registry for an agent's NFT and parsing its `tokenURI` to get off-chain interaction endpoints (A2A, MCP, ENS, agentWallet).

\* \*\*Session & Task Execution (Off-chain)\*\*: Using A2A for message and task orchestration (e.g., `message/send`, `message/stream` for real-time updates via SSE/gRPC streaming), and MCP for tool invocation and context provision. Authentication and security are handled at the transport layer.

\* \*\*On-chain Reputation\*\*: Clients provide feedback (score, tags, optional off-chain details with payment proof) to the Reputation Registry after receiving a `feedbackAuth` from the agent.

\* \*\*On-chain Validation\*\*: Agents can request validation for their work, and designated validators submit responses to the Validation Registry.

\* \*\*Wallet, Payment & Settlement\*\*: Wallets are central for signing transactions and authorizations. While ERC-8004 doesn't include payment, protocols like X402 can be integrated, with payment proofs recorded in feedback details.

\* \*\*End-to-End Flow\*\*: This entire cycle (Request → Execute → Verify → Feedback) creates a composable and auditable trust trajectory.

\#### 2. Building the Full Application Flow

The construction of this system involves aggregating various modules: Agent Capability, On-chain (Identity, Reputation, Validation Registries), A2A, MCP, Memory, Payment, Task Orchestration/Communication/Intent Matching/Power Delegation, Frontend, Privacy Inference, and Wallet Signing. Each module plays a critical role in realizing the vision of a system where intentions can be easily sent, discovered, discussed, executed, retried, split, summarized, replied to, and viewed.

\#### 3. The Roadmap

The ERC-8004 ecosystem is evolving rapidly. The roadmap outlines a transition from testing and toolchain refinement to mainnet deployment, cross-chain expansion, infrastructure, and product rollout.

\* \*\*Oct-Nov\*\*: Focus on developer experience, SDKs, documentation, and testnet validation.

\* \*\*Nov\*\*: Community events (Agentic Zero, Trustless Agents Day) and the launch of an "Ecosystem Map" for collaboration.

\* \*\*Nov-Dec\*\*: Mainnet deployment, cross-chain collaboration, and freezing the ERC-8004 specification.

\* \*\*Next 3-4 Weeks\*\*: Building core infrastructure like agent explorers, indexing, and reputation aggregation.

\* \*\*From Dec\*\*: Maturation of SaaS developer tools, connecting ERC-8004 with other protocols like X42, MCP, and A2A.

\* \*\*Product Line\*\*: Moving from prototypes to product-market fit, emphasizing real user engagement.

\* \*\*Public Communication\*\*: Engaging with the public, institutions, and regulators to articulate the unique value proposition of an open agent economy.
